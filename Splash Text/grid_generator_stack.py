...
def generate_grids(horizontal_word : str, vertical_word : str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    attempts : int = 0
    stack_best : int = CHAR_HEIGHT + 1
    voxels_best : np.ndarray = None
    ...
    while attempts < ATTEMPT_LIMIT:
        ...
        attempts += 1
        if stack_counts.max() < stack_best:
            stack_best = stack_counts.max()
            voxels_best = voxels.copy()
        if stack_counts.max() <= 1:
            break
        
    if stack_counts.max() > 1:
        logging.warning(f'Reached limit of {attempts} attempt(s)')
        logging.warning(f'Minimum stack count for "{horizontal_word}" and "{vertical_word}": {stack_best})')

    voxels = voxels_best.copy()
...
def main():
    ...
    for i in range(CHAR_HEIGHT):
        with open(f'L{i + 1}.txt', 'w') as f:
            binary : np.ndarray = (final_grid == i + 1).astype(int)
            f.write('\n'.join(''.join(str(c) for c in line) for line in binary))
